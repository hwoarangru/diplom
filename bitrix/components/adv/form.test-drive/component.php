...
*/
# увы, этот код работать не будет, т.к. для создания корректного события необходимо заполнить ВСЕ его поля
# в массиве arFields. тут же заполняется только 'EMAIL_TO'. чтобы убедиться, достаточно взглянуть на таблицу b_event
...
//Обзоры end
		...
		$arResult["QUESTIONS"]["first_Model"]["HTML_CODE"] = '<select name="form_text_'.$arResult["QUESTIONS"]["first_Model"]["STRUCTURE"]["0"]["ID"].'"><option value="">Пожалуйста,выберите модель...</option>'.$str."</select>";
		...
		if ($bCache)
		{
			...
			$obFormCache->EndDataCache(
                ...
                $arResult["QUESTIONS"]["info_brand"]["HTML_CODE"] = '<select name="form_text_'. $arResult["QUESTIONS"]["info_brand"]["STRUCTURE"]["0"]["ID"].'" id="info_brand" onChange = "selectChange();" class="inputSelect"><option value="">Пожалуйста выберите модель...</option>'.$arResult['SelectString'].'</select>';
