...
//pre($arResult['FORM_ERRORS']);

			if ((!isset($_POST["list"]))||($_POST["list"] == "")){
                              $flag = true;
                              if ($_POST["form_text_".$_SESSION["f_model"]] == "") $flag=false;
                              if ($flag){
                              	if(!isset($_POST["form_radio_f_type"])):
                              		$arResult['FORM_ERRORS']['f_model'] = "Не заполнены следующие обязательные поля: Модель";
                              	else:
		                              $arResult["list"] = 2;
		                              //$_SESSION["list"] = 2;
                              	...
                              	if((!isset($_POST["form_radio_f_type"]))||($_POST["form_radio_f_type"]!=$arResult['arAnswers']['f_type']['1']['ID'])):
                              		$arResult['FORM_ERRORS']['f_model'] = "Не заполнены следующие обязательные поля: Модель";
                              	else:
	                              $arResult["list"] = 2;
	                              $arResult['FORM_ERRORS'] = "";
                            ...
                            // echo('!!!!');
                             if ($_POST["list"] == "2"){
                              ...
                              if ((isset($arResult['FORM_ERRORS']["s1_family"]))||(isset($arResult['FORM_ERRORS']["s1_name"]))||(isset($arResult['FORM_ERRORS']["s1_surname"]))||(isset($arResult['FORM_ERRORS']["	s4_mphone"]))||(isset($arResult['FORM_ERRORS']["s5_email"]))||(isset($arResult['FORM_ERRORS']["s5_contact"]))) $flag = false;
                              if ($flag){
                              $arResult["list"] = 3;
                              //$_SESSION["list"] = 3;
                              $arResult['FORM_ERRORS'] = "";
            			...
            			/*pre($_POST);
            			pre($arResult['arAnswers']['f_type']['1']['ID']);
            			pre($arResult['FORM_ERRORS']);
            die();*/
		...
		*/
		...
		//echo '<pre>'; print_r($arResult['arForm']); echo '</>';
...
//начинаем выбирать модельки машин 
        $str="";
        ...
        while($ob = $res->GetNextElement())
               {
                  ...
                  if ($arFields['NAME'] == $_POST["form_text_".$arResult["QUESTIONS"]["first_Model"]["STRUCTURE"]["0"]["ID"]]){
                    $select = "selected";}
                 //Не учитываем обзоры + вывод в option
...
//Обзоры end
		...
		$arResult["QUESTIONS"]["f_model"]["HTML_CODE"] = '<select name="form_text_'.$arResult["QUESTIONS"]["f_model"]["STRUCTURE"]["0"]["ID"].'"><option value="">Пожалуйста,выберите модель...</option>'.$str."</select>";
		$_SESSION["f_model"] = $arResult["QUESTIONS"]["f_model"]["STRUCTURE"]["0"]["ID"];
        ...
        $arResult["QUESTIONS"]["info1_model"]["HTML_CODE"] = '<select name="form_text_'. $arResult["QUESTIONS"]["info1_model"]["STRUCTURE"]["0"]["ID"].'" id="info_brand" onChange = "selectChange();" class="inputSelect"><option value="">Пожалуйста выберите модель...</option>'.$arResult['SelectString'].'</select>';
        $arResult["QUESTIONS"]["info1_model2"]["HTML_CODE"] = '<select name="form_text_'. $arResult["QUESTIONS"]["info1_model2"]["STRUCTURE"]["0"]["ID"].'" id="info_model" class="inputSelect"></select>';
...
$arResult["date_id"] = $arResult["QUESTIONS"]["info3_birth"]["STRUCTURE"]["0"]["ID"];	
		$arResult["QUESTIONS"]["info3_birth"]["HTML_CODE"] = CalendarDate("form_date_".$arResult["QUESTIONS"]["info3_birth"]["STRUCTURE"]["0"]["ID"], $_POST["form_date_".$arResult["QUESTIONS"]["info3_birth"]["STRUCTURE"]["0"]["ID"]], "feedback");
		
		
                $arResult["info1_model"] = $arResult["QUESTIONS"]["info1_model"]["STRUCTURE"]["0"]["ID"];
                $arResult["info3_hobby"] = $arResult["QUESTIONS"]["info3_hobby"]["STRUCTURE"]["0"]["ID"];
                $arResult["info4_sphere"] = $arResult["QUESTIONS"]["info4_sphere"]["STRUCTURE"]["0"]["ID"];
                $arResult["info2_date"] = $arResult["QUESTIONS"]["info2_date"]["STRUCTURE"]["0"]["ID"];
                $arResult["info2_year"] = $arResult["QUESTIONS"]["info2_year"]["STRUCTURE"]["0"]["ID"];
                 $arResult["brand_id"] = $arResult["QUESTIONS"]["info1_model"]["STRUCTURE"]["0"]["ID"];
                $arResult["model_id"] = $arResult["QUESTIONS"]["info1_model2"]["STRUCTURE"]["0"]["ID"];
