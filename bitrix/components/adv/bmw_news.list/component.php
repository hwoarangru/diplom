...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		$arFilter_year=$arFilter;
		if(($_REQUEST['year']!="") and (($_REQUEST['year'] < date('Y')) and ($_REQUEST['year']>2000)))
		{
			$arNavParams["nPageSize"] = 200;
			$arFilter["><DATE_ACTIVE_FROM"] = Array(date($DB->DateFormatToPHP(CLang::GetDateFormat("FULL")), 
												mktime(0,0,0,1,1,intval($_REQUEST["year"]))),
											date($DB->DateFormatToPHP(CLang::GetDateFormat("FULL")), 
												mktime(0,0,0,1,1,(intval($_REQUEST["year"]+1)))));
		}
			
		$obParser = new CTextParser;
		$arResult["ITEMS"] = array();
		$arResult["ELEMENTS"] = array(); 
		if($arParams['SECTION_CODE']=='news'){
			$arsSFilter = Array('IBLOCK_TYPE'=>'news', 'IBLOCK_CODE'=>'news', 'CODE'=>$arParams['SECTION_CODE']);
			$dbs_list = CIBlockSection::GetList(Array($by=>$order), $arsSFilter, true);
			while($ar_dbs_result = $dbs_list->GetNext()){
				$arFilter['SECTION_ID'] = $ar_dbs_result['ID'];
			}
		}
		...
		$obdate = CIBlockElement::GetList($arSort, $arFilter_year, false, Array("nPageSize"=>50000), array("DATE_ACTIVE_FROM"));
		while($aryears = $obdate->GetNextElement())
		{
			$years = $aryears->GetFields();
			$pre_year=explode(" ", $years["DATE_ACTIVE_FROM"]);
			$year=explode(".", $pre_year[0]);
			if (!in_array($year[2], $arResult["YEARS"]))
				$arResult["YEARS"][] = $year[2];
		}
