...
function LogSave ($filename, $string, $enable_logging = true) {
	if($enable_logging) {
...
LogSave ('import.log', "\n\n\n".date ('d.m.Y H:i:s'));
LogSave ('import.log', "parse_a5.php starting...");
...
if(!isset($iBlockEnabled))
	...
	//$_FILES["filename"]["name"] = "prices/AVT_8.csv";
	$_FILES["filename"]["name"] = "prices/AVT_5.csv";

	LogSave ('import.log', "Parseing CSV file to array");
	// добавление элементов
	$values = ParseCsv($_SERVER["DOCUMENT_ROOT"].'/upload/'.$_FILES["filename"]["name"]);

	LogSave ('import.log', "Impoting info from array");
	$count = 0;
