...
if(!isset($iBlockEnabled))
	...
	$files_count = 1;
	while ($files_count < 4) {
		if (file_exists ($_SERVER["DOCUMENT_ROOT"].'/import/brt_5_temp_array_'.$files_count.'.php')) {
			require_once ($_SERVER["DOCUMENT_ROOT"].'/import/brt_5_temp_array_'.$files_count.'.php');
			...
			unlink ($_SERVER["DOCUMENT_ROOT"].'/import/brt_5_temp_array_'.$files_count.'.php');
			die ("done");
		}
		else
			$files_count++;
