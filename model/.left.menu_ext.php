...
if ($obMenuCache->StartDataCache($arParams["CACHE_TIME"], $CACHE_ID))
{
	...
	if (preg_match('/[A-Za-z]/', $arVariables['MODEL_ID']))
	{
		...
		$db_list = CIBlockElement::GetList(Array('sort'=>'asc'), $arFilter, false, false, array("CODE", "ID", "PROPERTY_GALLERY", "PROPERTY_FEATURE", "PROPERTY_SHOW_MEDIA_UNDER_TEC", "PROPERTY_NAME_MEDIA_FOR_MENU", "PROPERTY_SHOW_MAIN", "IBLOCK_SECTION_ID"));
		...
		if($show_main=='Y')
		{
			$arFilterm_sub = Array('IBLOCK_ID'=>192, 'ID'=>$section_code);// кто родитель
			$db_listm_sub = CIBlockSection::GetList(Array(), $arFilterm_sub, false, array("IBLOCK_SECTION_ID"));
			if($ar_resultm_sub = $db_listm_sub->GetNext())
			{
				$section_code_m = "sss".$ar_resultm_sub["IBLOCK_SECTION_ID"];
				$arFilterm = Array('IBLOCK_ID'=>192, 'ID'=>$ar_resultm_sub["IBLOCK_SECTION_ID"]); //какой у него код
				$db_listm = CIBlockSection::GetList(Array(), $arFilterm, false, array("CODE"));
				if($ar_resultm = $db_listm->GetNext())
				{
					//$section_code_m = $ar_resultm["CODE"];
				}
				else
				{
					$show_main = "N";
	...
	$rsSectionList = CIBlockSection::GetList($arOrder, $arFilter, false, array("ID", "DEPTH_LEVEL", "NAME", "SECTION_PAGE_URL"));
...
foreach ($arSections as $arSection)
 ...
 if (!$show_under) {   
     ...
     */
    $arOrder = array("SORT" => "ASC", "NAME" => "ASC");
    $arFilter = array("IBLOCK_CODE" => "technical_character", 'CODE' => $arVariables['CODE'], 'ACTIVE' => 'Y');
    $arSelect = array('NAME');
    $tcIBlockElementList = CIBlockElement::GetList($arOrder, $arFilter, false, Array("nPageSize"=>1), $arSelect);
